...
import { fetchNonStreaming, streamChat, uploadFile } from "../lib/api";
import type {
  ChatHistoryMessage,
  ChatRequestBody,
  ConversationTurn,
...
export function useChat() {
  ...
  const submit = useCallback(
    async (query: string, focusMode: FocusMode, file?: File, provider?: Provider) => {
  ...
  const retry = useCallback(
    (turnId: string) => {
      ...
      submit(turn.query, turn.focusMode, undefined, turn.provider);
    },
    [turns, submit]
  );
